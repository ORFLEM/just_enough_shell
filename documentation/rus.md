...
<div align="left">
	<h3>-- О проекте -- :</h3>
	<p>
	<i>JES</i> - rolling release desktop shell для wayland WMs,  поддерживающий подключение любого wm нативно, даже самописных.<br>
  <br>
	...
	<ul>
  	...
  	<li>Niri</li>
		<li>DriftWM</li>
		<li>Любой другой через плагин систему</li>
	</ul>
	<br>
	В проекте есть оптимизация, но он не тестировался на очень слабых пк.<br>
	Go бинарники используются для скриптов, где важна быстрая скорость считывания большого потока данных, за счёт этого нагрузка на ЦП в простое у <i>JES</i> - 1-2% ЦП и ~400мб ОЗУ, вместо 35-45%.<br>
	В данном репозитории есть референсные настройки, которые потребляют ~5-10% на цп при одномомниторном стеке и ~7-13% на многомониторном стеке (тестировалось на swayfx).<br>
  <br>
	...
	Монитор автора - UWQHD (3440x1440), проверенные разрешения: FHD (1920x1080) и выше.<br>
...
```
#### **Обои с скриншотов**: [тык](https://moewalls.com/vehicle/nissan-silvia-s13-gloomy-rain-live-wallpaper/)
...
## -- Комбинации клавиш для SwayfX, Hyprland и Niri в реф. настройках -- :
...
- для открытия центра плагинов надо нажать пкм по кнопке питания
- для открытия календаря надо нажать пкм по времени, лкм меняет время с датой местами
...
```
номер команды, дата, юзер, директория, состояние гита (при открытии проекта, связанный с git), код ошибки
...
- добавьте в imports `./JES.nix` и в конфиге
```nix
services.jes = {
  enable = true;
  users = [ " your user" ];
